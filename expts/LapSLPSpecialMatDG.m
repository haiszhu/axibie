function A = LapSLPSpecialMatDG(t,s)
...
for k=1:np
    j = (k-1)*s.p+(1:s.p); 
    panlen(k) = sum(s.ws(j)); 
    ik = (abs(t.x - s.xlo(k)) + abs(t.x - s.xhi(k))) < 1.3*panlen(k);
    if sum(ik)>0
        ...
        % 
        ss.Z = @(t) (s.Z((s.tlo(k)*(1-t)+s.thi(k)*(1+t))/2)-zmid)/zsc;
        ss.p = be*s.p; ss.tlo = -1; ss.thi = 1; ss = quadr(ss,ss.p,'p','G');
        Ak = S0formChebyquad(tt,ss,-1,1);
        Ak = Ak*abs(zsc) - log(abs(zsc))/(2*pi)*repmat(abs(wxpf)',[sum(ik) 1]);
        A(ik,ls+1:ls+len) = Ak*intMat;
...
function [A, A1, A2] = S0formChebyquad(t,s,a,b,interpMat,w0f)
