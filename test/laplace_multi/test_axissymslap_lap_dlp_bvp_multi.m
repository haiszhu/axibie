...
% MULTI-particle Laplace DLP exterior-Dirichlet BVP via the Fortran operators, CLOSE-CORRECTION,
% h-refinement of np (p fixed). COMBINED-FIELD representation (D+S)[sigma]: the +1/2 I exterior jump
% rides in the DLP self block, the SLP carries the monopole the DLP cannot represent -> full-rank,
% backslash (so the interior charges need NOT sum to zero).  Scalar (nc=1).
%   S (naive full = Lap3dDLPmat+Lap3dSLPmat, FMM-replaceable; diag 0) + Scorr (N self + N kdtree near diffs)
%   self  (D+S)_kk = Finv_k*(AbD_k+AbS_k)*F_k   (axp_physmat dlp + slp) ; near off-diag = (B_dlp+B_slp)*Fs
% Solve (S+Scorr) sigma = u_ex ; eval (D+S)[sigma] naive-full + per-source near close-correction.
...
yloc={[0.10 0.05; 0 0.05; 0.20 -0.30],[0.20 -0.10; 0 0.15; 0.10 -0.10]}; qq=[1.0 -0.7 0.6 -0.5];  % general (SLP carries the net charge)
...
for kk=1:numel(np_vals)
  ...
  % interior point charges inside each particle -> exact exterior field u_ex (general net charge; SLP carries it)
  ...
  Dsrc=@(xs,ws,ns) struct('x',xs,'w',ws,'nx',ns);                 % DLP source struct (needs normals)
  Dnv=@(t,xs,ws,ns) Lap3dDLPmat(t, Dsrc(xs,ws,ns)) + Lap3dSLPmat(t, struct('x',xs,'w',ws));  % combined (D+S) naive

  % --- (1) naive FULL (D+S) operator (FMM-replaceable); diagonal zeroed ---
  S=Dnv(struct('x',Xall), Xall, wall, Nall); S(1:Mtot+1:end)=0;
  Scorr=sparse(Mtot,Mtot);

  % --- (2) self DIFFERENCE correction: (accurate (D+S) self) - (naive (D+S) self, diag 0) ---
  for k=1:2
    g=geo{k};
    [AbD,F,Finv,~,~]=axp_physmat_mex(1,3,1,p,np,g.sx,g.snx,g.sws,g.swxp,g.tpan,g.sxlo,g.sxhi,M,iside,iclosed,1.0,eye(3)); % +1/2 I + D
    [AbS,~,~  ,~,~]=axp_physmat_mex(1,1,1,p,np,g.sx,g.snx,g.sws,g.swxp,g.tpan,g.sxlo,g.sxhi,M,iside,iclosed,1.0,eye(3)); % S
    Sns=Dnv(struct('x',X{k}), X{k}, sq{k}.w, NX{k}); Sns(1:N+1:end)=0;
    Scorr(blk{k},blk{k})=Scorr(blk{k},blk{k})+(real(Finv*(AbD+AbS)*F)-Sns);
  end

  % --- (3) off-diagonal near DIFFERENCE correction (kdtree): (D+S) close - naive ---
  for pq=1:2
    ...
    nr=cand(nmask); Bf=real((BD+BS)*Fs);
    Sno=Dnv(struct('x',Xall(:,nr)), X{pq}, sq{pq}.w, NX{pq});
    Scorr(nr,blk{pq})=Scorr(nr,blk{pq})+(Bf(nmask,:)-Sno);
  end

  % --- Dirichlet data + solve ---
  ...
  % --- field eval: naive (D+S) + per-source near close correction ---
  ...
  xlabel('x'); ylabel('y'); zlabel('z'); title('multi-particle Laplace DLP (D+S, close-correction): 3D field error (last refinement)');
end

% --- convergence plot ---
figure(1),clf; semilogy(np_vals,errmax,'o-k'); grid on; xlabel('n_p'); ylabel('max field err');
title('multi-particle Laplace DLP (D+S, close-correction): h-refinement, p=16, M=2 n_p');

% cmp = getPyPlot_cMap('rainbow', [], [], '"/Users/hzhu/.pyenv/versions/3.11.13/bin/python"');
% colormap(cmp)
% 
% exportgraphics(figure(1),'axissymslap_lap_dlp_multi_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymslap_lap_dlp_multi_error.png','Resolution',200)