...
% MULTI-particle Laplace SLPn (S') exterior-Neumann BVP via the Fortran operators, CLOSE-CORRECTION,
% h-refinement of np. Mirror omega3 test_LapSLPnAxiPhysMat0: single-layer representation u = S[sigma],
% match the surface flux S'[sigma] = dn(u_ex) (the -1/2 exterior jump rides in the SLPn self block;
% second-kind, full-rank -> backslash).  Scalar (nc=1).
%   SYSTEM (flux): S = naive Lap3dSLPmat-normalderiv (FMM-replaceable, diag 0) + Scorr (N self + N near diffs)
%     self  S'_kk = Finv_k*Ab_k*F_k       (axp_physmat slpn) ; near off-diag = B_slpn*Fs (target normals)
%   EVAL (potential): u = S[sigma] -> naive Lap3dSLPmat + SLP value close-correction (same as the SLP test).
% Solve (S+Scorr) sigma = dn(u_ex) ; evaluate the SLP potential field.
