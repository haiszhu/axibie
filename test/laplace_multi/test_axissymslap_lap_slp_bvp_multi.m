...
% MULTI-particle Laplace SLP Dirichlet BVP via the Fortran operators, CLOSE-CORRECTION architecture,
% over an h-refinement of the panel count np (p fixed):
%   S (naive full / FMM-replaceable, untouched)  +  Scorr (sparse: N self diffs + N kdtree near off-diag diffs)
%   solve (S+Scorr) sigma = u_ex ; evaluate naive-full + per-source near close-correction.
