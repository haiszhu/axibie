...
yloc={[0.10 0.05; 0 0.05; 0.20 -0.30],[0.20 -0.10; 0 0.15; 0.10 -0.10]}; qq=[1.0 -0.7 0.6 -0.5];  % general net charge (SLP carries it)
...
for kk=1:numel(np_vals)
  ...
  % interior point charges inside each particle -> exact exterior field uex (Dirichlet data on Xall)
  ...
  for k=1:K2
    ...
    [bg, ig, tg] = axpso_corr_get_mex(hDself, k, nt, 1, p, np, M, zeros(nt*nphi*p,1), zeros(nt,1), zeros(np+1,1));
    fprintf('  DLP self block k=%d:  %d x %d  (ntcx=%d)\n', k, nt, nphi*p, nt);
  end

  % 4. matrix-free operator: lfmm3d naive (D+S) + four corr applies (scalar, no deflation)
  applyA=@(x) applymvDS(x, fmmeps, Xall, Nall, wall, hDself, hDcross, hSself, hScross);

  % 5. solve (gmres, combined-field second-kind full-rank -> no deflation)
  [sigma,flag,relres,iter]=gmres(applyA, g_di, [], 1e-14, 300);
  ...
  % 7. field eval u = (D+S)[sigma]: lfmm3d naive (D+S) at Pe + {D,S} eval corrections (targoff=ones ->
  ...
  xlabel('x'); ylabel('y'); zlabel('z'); title('2-particle Laplace DLP (D+S, physmat sparse-correction): 3D field error (last refinement)');
  figure(2),ylim([-1 3])
...
title('multi-particle Laplace DLP (D+S, physmat sparse-correction): h-refinement, p=16, M=2 n_p');

% cmp = getPyPlot_cMap('rainbow', [], [], '"/Users/hzhu/.pyenv/versions/3.11.13/bin/python"');
% colormap(cmp)

% exportgraphics(figure(1),'axissymslap_lap_dlp_multi_physmat_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymslap_lap_dlp_multi_physmat_error.png','Resolution',200)

function y = applymvDS(x, fmmeps, Xall, Nall, wall, hDself, hDcross, hSself, hScross)
% matrix-free (+1/2 I + D + S) matvec: lfmm3d naive (D+S) = charges (S) + dipoles (D) at the sources
% (self i~=j excluded; +1/2 jump in the DLP self correction) + four handle corr applies (ACCUMULATE).
cw=(x(:).').*wall;                                            % charge/dipole strength (weights baked in)
srcinfo=[]; srcinfo.sources=Xall; srcinfo.charges=cw; srcinfo.dipoles=Nall.*cw;
U=lfmm3d(fmmeps,srcinfo,1); y=U.pot(:);                       % (D+S)[sigma] potential at the sources
n=numel(x);
