...
% ---- geometry switch (mirror axibie/testStokesDir.m) ----
...
for kk=1:numel(np_vals)
  ...
  % 1. geometry + Neumann (dn u_ex) data setup
  ...
  % 2. self modal matrix (-1/2 I + S')  (target normal = surface normal)
  % FROZEN worker path (uncomment to compare): Aself=axls_slpn_blockmat_nmode_mex(N,sx,snx,p,np,sx,snx,sws,swxp,tpan,sxlo,sxhi,pmodes,iside,iclosed,[]);
  Aself=real(axp_modemat_setup_mex(1,2,0,1,1,p,np,pmodes,iside,iclosed,[1;N+1],[1;np+2],N,np+1, ...
      sx,snx,sws,swxp,tpan,0,[1;1],zeros(3,0),zeros(3,0),eye(3),zeros(3,1),N,N));

  % 3. solve  (-1/2 I + S'_m) sigma_m = dn u_m
  sig=cell(pmodes+1,1);
  for m=0:pmodes
    sig{m+1}=Aself(:,:,m+1)\gm(m+pmodes+1,:).';
  end
  ...
  % 5. log10 error  (exterior Neumann is unique -> absolute, no constant gauge)
  Uex3=uex(P); err3=abs(u3-Uex3)/max(abs(Uex3));
  inb=d3<0.1; errmax(kk)=max(err3);
  fprintf('Laplace SLPn all-modes Neumann BVP [%s] (delta mex): N=%d, np=%d, pmodes=%d, M3=%d\n',shape,N,np,pmodes,M3);
...
title(sprintf('Laplace SLPn all-modes Neumann BVP [%s]: h-refinement, p_{modes}=2 n_p',shape));
...
xlabel('x'); ylabel('y'); zlabel('z'); title(sprintf('Laplace SLPn all-modes Neumann [%s]: 3D target-grid potential error',shape));
ylim([-2 3])

% cmp = getPyPlot_cMap('rainbow', [], [], '"/Users/hzhu/.pyenv/versions/3.11.13/bin/python"');
% colormap(cmp)

% exportgraphics(figure(1),'axissymslap_lap_slpn_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymslap_lap_slpn_error.png','Resolution',200)
