...
for kk=1:numel(np_vals)
  ...
  fprintf('Laplace SLPn all-modes Neumann BVP (delta mex): N=%d, np=%d, pmodes=%d, M3=%d\n',N,np,pmodes,M3);
...
title('Laplace SLPn all-modes Neumann BVP (delta mex): h-refinement, p_{modes}=2 n_p');
...
xlabel('x'); ylabel('y'); zlabel('z'); title('Laplace SLPn all-modes Neumann: 3D target-grid potential error');
