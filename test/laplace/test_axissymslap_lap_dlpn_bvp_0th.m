...
for k=1:numel(Np)
  ...
  g=gexact(s);
  % FROZEN single-mode path (uncomment to compare): Sp=axls_slpn_blockmat_mex(N,sx,snx,p,np,sx,snx,sws,swxp,tpan,sxlo,sxhi,iside,iclosed,[]);
  Sp=real(axp_modemat_setup_mex(1,2,0,1,1,p,np,0,iside,iclosed,[1;N+1],[1;np+2],N,np+1, ...
      sx,snx,sws,swxp,tpan,0,[1;1],zeros(3,0),zeros(3,0),eye(3),zeros(3,1),N,N));
  % FROZEN single-mode path (uncomment to compare): Dp=axls_dlpn_blockmat_mex(N,sx,snx,p,np,sx,snx,sws,swxp,tpan,sxlo,sxhi,iside,iclosed,[]);
  Dp=real(axp_modemat_setup_mex(1,4,0,1,1,p,np,0,iside,iclosed,[1;N+1],[1;np+2],N,np+1, ...
      sx,snx,sws,swxp,tpan,0,[1;1],zeros(3,0),zeros(3,0),eye(3),zeros(3,1),N,N));
  dens=(Sp+Dp)\g;
