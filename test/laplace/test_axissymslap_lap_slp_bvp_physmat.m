...
for kk=1:numel(np_vals)
  ...
  Aself = As;                                         % SLP-only (1st-kind): ignore Ad
  for j = 1:np
    idxj = (j-1)*p + (1:p);
  ...
  % 5. log10 error
  Uex3=uex(P); 
  fprintf('Laplace SLP (1st-kind) all-modes Dirichlet BVP [%s] (delta mex): N=%d, np=%d, pmodes=%d, M3=%d\n',shape,N,np,pmodes,M3);
...
title(sprintf('Laplace SLP (1st-kind) all-modes Dirichlet BVP [%s]: h-refinement, p_{modes}=2 n_p',shape));
...
xlabel('x'); ylabel('y'); zlabel('z'); title(sprintf('Laplace SLP (1st-kind) all-modes [%s]: 3D target-grid potential error',shape));
ylim([-2 3])
