...
% 3. h-refinement: Neumann solve (-1/2 I + S') sigma = dn u, eval single-layer potential S[sigma]
err=nan(size(Np));
for k=1:numel(Np)
  ...
  g=gexact(s);                                          % Neumann data = exact dn u (surface normal)
  % FROZEN single-mode path (uncomment to compare): A =axls_slpn_blockmat_mex(N,sx,snx,p,np,sx,snx,sws,swxp,tpan,sxlo,sxhi,iside,iclosed,[]);
  A =real(axp_modemat_setup_mex(1,2,0,1,1,p,np,0,iside,iclosed,[1;N+1],[1;np+2],N,np+1, ...
  ...
  u=nan*zz; u(ii)=tt;
  err(k)=max(abs(u(:)-uf(:)));
  fprintf('Laplace SLPn 0th-mode Neumann BVP (delta mex): np=%2d, N=%d: max err vs exact = %.3e\n',Np(k),N,err(k));
end

% 4. meridian error map + convergence
figure(1); clf; semilogy(Np,err,'o-k'); xlabel('n_p'); ylabel('max err'); grid on;
title('Laplace SLPn 0th-mode Neumann BVP: h-refinement');
...
xlabel('\rho'); ylabel('z'); title('Laplace SLPn 0th-mode: log_{10} err vs exact axi-ring potential');

% cmp = getPyPlot_cMap('rainbow', [], [], '"/Users/hzhu/.pyenv/versions/3.11.13/bin/python"');
% colormap(cmp)

% exportgraphics(figure(1),'axissymslap_lap_slpn0th_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymslap_lap_slpn0th_error.png','Resolution',200)
...
%   grad Phi_ring(x) = sum_j (q_j/2pi) int_0^2pi grad_x 1/(4pi|x-y(phi)|) dphi  (y on the ring, |x| meridian).
