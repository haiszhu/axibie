...
for kk=1:numel(np_vals)
  ...
  % 2. self modal matrix
  % FROZEN worker path (uncomment to compare): Aself=axss_slp_blockmat_nmode_mex(N,sx,p,np,sx,snx,sws,swxp,tpan,sxlo,sxhi,pmodes,iside,iclosed,mu,[]);
  Aself=axp_modemat_setup_mex(2,1,mu,1,1,p,np,pmodes,iside,iclosed,[1;N+1],[1;np+2],N,np+1, ...
      sx,snx,sws,swxp,tpan,0,[1;1],zeros(3,0),zeros(3,0),eye(3),zeros(3,1),3*N,3*N);
  % Aself=axss_slp_blockmat_nmode(N,sx,p,np,sx,snx,sws,swxp,tpan,sxlo,sxhi,pmodes,iside,iclosed,mu,[]);
