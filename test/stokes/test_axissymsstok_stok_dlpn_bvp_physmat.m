...
% combined field u = (D_+ + S)[sigma]; solve the traction Neumann system (-1/2 I + S' + D') sigma = t.
...
errmax=nan(1,numel(np_vals));           % near velocity (post-eval of u = (S+D)[sigma])
err_trac=nan(1,numel(np_vals));         % traction referee ((S'+D')[sigma] at random 3D normals)
gate = 2.0;
for kk=1:numel(np_vals)
  ...
  % 2. per-panel SELF system matrix (-1/2 I + S' + D'): axps_naivestoksdlpn_physmat
  ...
  %    in S'; S' regularizes the hypersingular D' -> full rank): wrapper evaluated ONCE
  %    at theta=0, written into ALL nang azimuth row-blocks with the circulant column
  %    shift AND the rotz conjugation W_a = R(phi_a) W0 R(phi_a)^T
  ...
  % 3. solve the traction Neumann system (dense direct backslash)
  sigphys = A\tphys;

  % 4. per-panel post-eval of the VELOCITY u = (S + D)[sigma] (explicit naive modal matrix +
  ...
  Atargs = Astargs + Adtargs;                          % naive (S + D) velocity physical matrix
  for j = 1:np
    idxj = (j-1)*p + (1:p);
  ...
  % 5. traction referee: t = (S' + D')[sigma] at RANDOM 3D target normals (n_theta ~= 0),
  ...
  Atn = Asn + Adn;                                     % naive (S' + D') traction physical matrix
  for j = 1:np
    idxj = (j-1)*p + (1:p);
    Tj = find(abs(ttx - sxlo(j)) + abs(ttx - sxhi(j)) < 1.85*sum(sws(idxj)));
    if isempty(Tj), continue, end
  ...
  % 6. errors
  Uex3 = uex(P);
  fprintf('Stokes (-1/2 I + S'' + D'') traction Neumann BVP [%s] (per-panel): N=%d, np=%d, pmodes=%d, M3=%d\n',shape,N,np,pmodes,M3);
  err3p = vecnorm(u3p-Uex3)/max(vecnorm(Uex3));
  errmax(kk)=max(err3p);
  fprintf('  velocity  (S+D) eval over ALL %d targets: max err = %.3e\n', M3, max(err3p));
  errT = vecnorm(t3p-Tex)/max(vecnorm(Tex));
  err_trac(kk)=max(errT);
  fprintf('  traction (S''+D'') referee, random 3D n over ALL %d targets: max err = %.3e\n', M3, max(errT));
...
xlabel('n_p'); ylabel('max err'); grid on; legend('velocity (S+D)','traction (S''+D'')','Location','best');
title(sprintf('Stokes (-1/2 I + S''+D'') traction Neumann BVP [%s] (per-panel): h-refinement',shape));
