...
% 3. h-refinement: Neumann solve (-1/2 I + S') mu = sigma.n, eval single-layer velocity S[mu]
err=nan(size(Np));
for k=1:numel(Np)
  ...
  err(k)=max(abs(u(:)-uf(:)));
  fprintf('Stokes SLPn 0th-mode Neumann BVP (delta mex): np=%2d, N=%d: max err vs exact = %.3e\n',Np(k),N,err(k));
end

% 4. meridian error map + convergence
figure(1); clf; semilogy(Np,err,'o-k'); xlabel('n_p'); ylabel('max err'); grid on;
title('Stokes SLPn 0th-mode Neumann BVP (delta mex): h-refinement');
...
xlabel('\rho'); ylabel('z'); title('Stokes SLPn 0th-mode: log_{10} err vs exact axi-Stokeslet velocity');

% exportgraphics(figure(1),'axissymsstok_stok_slpn0th_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymsstok_stok_slpn0th_error.png','Resolution',200)
