...
for kk=1:numel(np_vals)
  ...
  % 1. geometry + Neumann (traction sigma.n) data setup
  ...
  for m=0:pmodes
    ...
    dens{m+1}=lsqminnorm(Aslpn(:,:,m+1),rhs);
  end
  ...
  % 5. 3d target eval (single-layer velocity S[mu])
  ...
  uz3(isnear)=uzn;
  % far
  SR = cell2mat(cellfun(@(c) c(1:N).',      dens,'uni',0));
  SP = cell2mat(cellfun(@(c) c(N+1:2*N).',  dens,'uni',0));
  SZ = cell2mat(cellfun(@(c) c(2*N+1:3*N).',dens,'uni',0));
  ...
  fprintf('Stokes SLPn all-modes Neumann BVP (delta mex): N=%d, np=%d, pmodes=%d, M3=%d\n',N,np,pmodes,M3);
...
title('Stokes SLPn all-modes Neumann BVP (delta mex): h-refinement, p_{modes}=2 n_p');
...
xlabel('x'); ylabel('y'); zlabel('z'); title('Stokes SLPn Neumann all-modes: 3D target-grid velocity error');

% exportgraphics(figure(1),'axissymsstok_stok_slpn_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymsstok_stok_slpn_error.png','Resolution',200)
