...
% 1st-kind single-layer ansatz u = S[sigma] (SLP value is continuous: no jump term)
...
for kk=1:numel(np_vals)
  ...
  % 2. per-panel SELF system matrix (S): axps_naivestoksdlp_physmat (theta=0
  ...
  %    (axps_closestoksdlp_panel_mex, SLP value is continuous: no jump term):
  %    wrapper evaluated ONCE at theta=0, written into ALL nang azimuth
  ...
  % nblk = 3*N*nang;
  % Aself = axp_physmat_setup_mex(2,1,mu,1,1,p,np,pmodes,iside,iclosed,[1;N+1],[1;np+2],N,np+1, ...
  ...
  Aself = Aselfs;                                       % SLP-only (1st-kind): ignore Aselfd
  for j = 1:np
    idxj = (j-1)*p + (1:p);
  ...
  % 6. solve (dense direct backslash, 1st-kind single-layer)
  ...
  Atargs = Ats;                                         % SLP-only: ignore Atd
  %%%%%
  for j = 1:np
    idxj = (j-1)*p + (1:p);
  ...
  % 4. log10 error
  Uex3=uex(P);
  fprintf('Stokes SLP (1st-kind) all-modes Dirichlet BVP [%s] (per-panel): N=%d, np=%d, pmodes=%d, M3=%d\n',shape,N,np,pmodes,M3);
...
title(sprintf('Stokes SLP (1st-kind) all-modes Dirichlet BVP [%s]: h-refinement, p_{modes}=2 n_p',shape));
...
xlabel('x'); ylabel('y'); zlabel('z'); title(sprintf('Stokes SLP (1st-kind) all-modes [%s]: 3D target-grid velocity error',shape));
ylim([-2 3])
...
% exportgraphics(figure(1),'axissymstok_stok_slp_convergence.png','Resolution',200)
% exportgraphics(figure(2),'axissymstok_stok_slp_error.png','Resolution',200)
