function A = LapDLPAxiSpecialMat(t,s)
% special quadrature matrix for axisymmetric Laplace double-layer (DLP).
%
% DLP kernel  (1/4pi) n_y.(x-y)/|x-y|^3   (normal at the SOURCE y).  Close
% evaluation by the same kernel-split scheme as LapGradAxiSpecialMat.m (the
% S'/adjoint DLP), with target normal -> source normal:
%   n_rho/rho -> n_rho'/rho',  m -> m',  and the Cauchy term sign flipped.
% See latex/laplace_dlp_axi.tex (blocks B04-B11).
%
% Needs s.nx (source normal); does NOT use a target normal.
%
% Hai (DLP analog of LapGradAxiSpecialMat)
...
for k=1:s_aux.np
    ...
    tfar = []; tfar.x = t.x(~ik(:));
    x1 = real(tfar.x(:)); x2 = imag(tfar.x(:));      % target (column)
    y1 = real(sk.x(:))'; y2 = imag(sk.x(:))';        % source (row)
    ny1 = real(sk.nx(:))'; ny2 = imag(sk.nx(:))';    % source normal (row)
    ...
    mp = bsxfun(@minus,bsxfun(@times,ny1,x1),bsxfun(@times,ny1,y1)) ...
       + bsxfun(@minus,bsxfun(@times,ny2,x2),bsxfun(@times,ny2,y2));   % m'
    S  = -bsxfun(@times,sqrty1x1,ny1./y1);                            % s_K
    SE = ( bsxfun(@times,sqrty1x1,ny1./y1) + 2*sqrty1x1.*mp./r2 );    % s_E  (+ sign vs S')
...
% axisymmtric Laplace DLP special quadrature on one panel...
...
% log-singular (EllipticK) part, close evaluation, one panel.  source normal.
...
ny1 = real(sf.nx(:))';                        % source normal (row)
...
% smooth part associated with EllipticK, one panel.  source normal.
...
% EllipticE part (smooth + log + Cauchy), one panel.  source normal, + Cauchy.

be = 2; % upsampling factor...
sf = quadr_panf(s,be,'G');
x1 = real(t.x(:)); x2 = imag(t.x(:));                 % target (column)
y1 = real(sf.x(:))'; y2 = imag(sf.x(:))';             % source (row)
ny1 = real(sf.nx(:))'; ny2 = imag(sf.nx(:))';         % source normal (row)
r2 = bsxfun(@minus,x1,y1).^2+bsxfun(@minus,x2,y2).^2;
chi = 1+r2./(2*bsxfun(@times,x1,y1)); tmp = 2./(chi+1);
[~,Smat1] = ellipke(tmp); Smat1 = sqrt(tmp).*Smat1;   % sqrt(2/(chi+1)) E
...
Rhat = Smat1+Smat2./(chi+1);                          % Rbar
mp = bsxfun(@minus,bsxfun(@times,ny1,x1),bsxfun(@times,ny1,y1)) ...
   + bsxfun(@minus,bsxfun(@times,ny2,x2),bsxfun(@times,ny2,y2));   % m'
% log density: ( n_rho'/rho' * r2 + 2 m' )/(chi+1) * beta_E   (+ sign vs S')
Smatlog = sqrty1x1.*( bsxfun(@times,ny1./y1,r2)+2*mp )./(chi+1).*TrLel;

% need to do 'log', 'smooth', 'cauchy' at the same time...
Asmooth = bsxfun(@times,sqrty1x1,ny1./y1).*Rhat*diag(sf.ws)*interpmat(s.p,sf.p,'G');
% Cauchy: numerator normal is the SOURCE normal (=> ny./sf.nx, combining to 1),
% with +4*pi (sign flipped vs S' -4*pi) for the DLP +2m'/r2 Cauchy term.
Acauchy1 = 4*pi*(DspecialMat.*bsxfun(@rdivide,ny1,sf.nx.')).*sqrty1x1.*Rhat;
Acauchy2 = 4*pi*(DspecialMat.*bsxfun(@rdivide,1i*ny2,sf.nx.')).*sqrty1x1.*Rhat;
...
% truncated Legendre function (EllipticK).  tx: target; sx: source.

x1 = real(tx(:)); x2 = imag(tx(:));     % target (column)
y1 = real(sx(:))'; y2 = imag(sx(:))';   % source (row)
...
% truncated Legendre function for EllipticE.  tx: target; sx: source.
% A for the smooth part, Al for the log-associated part.
