function A = LapAxiSpecialMat(t,s)
% special quadrature matrix for axisymmetric Laplace ....
%
%
%
% Hai 05/16/21
...
for k=1:s_aux.np
    ...
    Ak = AxiLaplaceSpecialquad(tk,sk);    
    ...
    S = sqrty1x1*2;
    AfarK = 1/(4*pi)*(SnaiveK.*S)*diag(sk.ws);
    
    % add contribution of kth panel to close and far targets...
    Amat( ik(:),pidx) = Amat( ik(:),pidx) + Ak;
...
function A = AxiLaplaceSpecialquad(t,s)
