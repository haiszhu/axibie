...
for k=1:numel(Np)
    ...
    % use SLP + DLP to solve a Dirichlet problem 
    As = LapAxiSpecialMat(s,s); 
    Ad = LapDLPAxiSpecialMat(s,s); 
    rhs = LapAxiMat(s,y_source)*pt_force;
    tau = (As+Ad)\rhs;

    % use SLP + DLP to evaluate soln
    us = nan*xx; ud = nan*xx; u = nan*xx;
