function [As3d, Ad3d] = axps_naivelapsdlp_physmat_mex(nt, tx, t3dx, p, np, nang, sx, snx, sws, pmodes, ifself, nrA, As3d, Ad3d)
nt=double(nt); p=double(p); np=double(np); nang=double(nang); M=double(pmodes); ifself=double(ifself(1)); nrA=double(nrA(1));
tx=double(tx(:)); t3dx=double(t3dx); sx=double(sx(:)); N=np*p; Nnang=N*nang;
...
mex_id_ = 'axps_naivelapsdlp_physmat_r64(c i int64_t[x], c i dcomplex[x], c i double[xx], c i int64_t[x], c i int64_t[x], c i int64_t[x], c i dcomplex[x], c i dcomplex[x], c i double[x], c i int64_t[x], c i int64_t[x], c i int64_t[x], c io double[xx], c io double[xx])';
