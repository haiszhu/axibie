...
contains
  ...
  ! All-modes carrier: fills vk(0:M)=Q_{n-1/2}, ve, Fn=P_{n-1/2}, An, dFn for n=0..M in ONE pass --
  ! single elliptic (AGM) eval per block and a single forward + single backward (Miller) recurrence sweep,
  ! replacing M+1 separate carrier_r64 calls (each of which re-did the AGM and an O(6n) Miller sweep per mode).
  ! Same values as carrier_r64 (low modes bit-identical via the forward recurrence; high modes machine-precision
  ! equal -- the Miller sweep here runs to Mtop(M) >= Mtop(n), i.e. at least as converged).
  subroutine carrier_all_r64(chi, M, vk, ve, Fn, An, dFn)
    real(r64),  intent(in)    :: chi
  ...
  end subroutine carrier_all_r64
