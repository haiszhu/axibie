...
! SLPn close-eval block (mer W + swirl Wsw).  TARGET normal nxr,nxi (length nt).
subroutine axm_specialquad_slpn_r64(nt, p, M, mu, hm, iside, zr, zi, nxr, nxi, yr, yi, ypr, ypi, &
    zar, zai, zbr, zbi, gw, Pmat, W, Wsw)
  use axissymstok_specialquad_mod, only: slpn => axissymstok_specialquad_slpn_r64
  implicit none
  integer(8), intent(in)    :: nt, p, M, iside
  real(8),    intent(in)    :: mu, hm, zar, zai, zbr, zbi
  real(8),    intent(in)    :: zr(nt), zi(nt), nxr(nt), nxi(nt)
  real(8),    intent(in)    :: yr(2*p), yi(2*p), ypr(2*p), ypi(2*p)
  real(8),    intent(in)    :: gw(2*p), Pmat(2*p,p)
  real(8),    intent(inout) :: W(M+1,3*nt,3*p), Wsw(M+1,3*nt,3*p)
  complex(8) :: z(nt), nx(nt), y(2*p), yp(2*p), za, zb
  integer(8) :: k
  do k = 1, nt
    z(k)  = cmplx(zr(k),  zi(k),  8)
    nx(k) = cmplx(nxr(k), nxi(k), 8)
  end do
  do k = 1, 2*p
    y(k)  = cmplx(yr(k),  yi(k),  8)
    yp(k) = cmplx(ypr(k), ypi(k), 8)
  ...
  za = cmplx(zar, zai, 8); zb = cmplx(zbr, zbi, 8)
  call slpn(nt, z, nx, p, hm, y, yp, za, zb, gw, Pmat, M, mu, iside, W, Wsw)
end subroutine axm_specialquad_slpn_r64
