...
subroutine axss_slp_blockmat_nmode_r64(nt, tx, p, np, sx, snx, sws, swxp, tpan, sxlo, sxhi, M, iside, iclosed, mu, A, nskip, skippanidx)
  ...
  call blk(nt, tx, p, np, sx, snx, sws, swxp, tpan, sxlo, sxhi, M, iside, iclosed, mu, A, nskip, skippanidx)
...
subroutine axss_slpn_blockmat_nmode_r64(nt, tx, tnx, tnphi, p, np, sx, snx, sws, swxp, tpan, sxlo, sxhi, M, iside, iclosed, mu, A)
  use axissymstok_specialquad_mod, only: blk => axissymstok_slpn_blockmat_nmode_r64
  implicit none
  integer(8), intent(in)    :: nt, p, np, M, iside, iclosed
  complex(8), intent(in)    :: tx(nt), tnx(nt), sx(p*np), snx(p*np), swxp(p*np), sxlo(np), sxhi(np)
  real(8),    intent(in)    :: sws(p*np), tpan(np+1), mu, tnphi(nt)
  complex(8), intent(inout) :: A(3*nt, 3*np*p, M+1)
  call blk(nt, tx, tnx, tnphi, p, np, sx, snx, sws, swxp, tpan, sxlo, sxhi, M, iside, iclosed, mu, A)
...
subroutine axss_dlp_blockmat_nmode_r64(nt, tx, p, np, sx, snx, sws, swxp, tpan, sxlo, sxhi, M, iside, iclosed, mu, A, nskip, skippanidx)
...
subroutine axss_dlpn_blockmat_nmode_r64(nt, tx, tnx, tnphi, p, np, sx, snx, sws, swxp, tpan, sxlo, sxhi, M, iside, iclosed, mu, A)
  use axissymstok_specialquad_mod, only: blk => axissymstok_dlpn_blockmat_nmode_r64
  implicit none
  integer(8), intent(in)    :: nt, p, np, M, iside, iclosed
  complex(8), intent(in)    :: tx(nt), tnx(nt), sx(p*np), snx(p*np), swxp(p*np), sxlo(np), sxhi(np)
  real(8),    intent(in)    :: sws(p*np), tpan(np+1), mu, tnphi(nt)
