...
# the support library build/libaxissymstok_kernelsplit.a, which is shipped prebuilt together
# with its module interface build/axissymstok_kernelsplit_mod.mod (the .mod lets the sources
# that `use` that module compile; the .a provides the compiled routines at link time).
...
# Fortran sources compiled here.  axissymstok_specialquad_{mod,mex} + axissymstok_kernelsplit_mex
# use the axissymstok_kernelsplit module -> they compile against build/axissymstok_kernelsplit_mod.mod
# and link build/libaxissymstok_kernelsplit.a.
...
all:
	...
	@echo "  NOTE: build/libaxissymstok_kernelsplit.a + axissymstok_kernelsplit_mod.mod must be present."
...
$(MEX_OUT): $(PUB_LIB) $(SEC_LIB) $(MEX_C)
	$(CC) $(MEX_LDFLAGS) -fPIC \
	  -DMATLAB_MEX_FILE -DMATLAB_DEFAULT_RELEASE=R2018a -DMX_COMPAT_32=0 \
	  -I$(MATLAB_ROOT)/extern/include \
	  $(MEX_C) \
	  -L$(BLD_DIR) -l$(PROJECT) -laxissymstok_kernelsplit \
