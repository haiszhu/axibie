...
for i = 2:s.np-1
    ...
    for j = 1:p
        ...
        % Alpert quadr
        tA = struct('x',s.x(indj),'nx',s.nx(indj)); % target: j-th local node
        ...
        for c=1:3 % loop over 3 local panels of sources (interp from same panel)
            ...
            xc = xP(indc); spc = spP(indc); nxc = nxP(indc);
            ...
            aws = aw(iA).*asp; 
            anx = baryeval(tInfl(:,ic)*2/tpanlenInfl(ic), stdbw, nxc, at(iA)*2/tpanlenInfl(ic));
            sA = struct('x',ax,'ws',aws,'w',aw(iA),'sp',asp,'nx',anx); % Alpert sourse;
%             matA = SLPmatrix(tA,sA);
            matA = SphereQuadKtp(sA,tA);
...
i = 1; % 1st panel
t1 = struct('x',s.x(1:p),'nx',s.nx(1:p)); s_aux1 = struct('x',s_aux.x(1:(n_split+1)*p),'ws',s_aux.ws(1:(n_split+1)*p),'nx',s_aux.nx(1:(n_split+1)*p)); % Alpert source;
% A_aux1 = SLPmatrix(t1,s_aux1);  A_aux1_0 = A_aux1;  
A_aux1 = SphereQuadKtp(s_aux1,t1); A_aux1_0 = A_aux1; 
for j=1:p
    indj = (i-1)*p+j; % global index of the j-th local node
    ...
    for c=1:3 % loop over 3 local panels of sources (interp from same panel)
        ...
        aws = aw(iA).*asp; 
%         anx = baryeval(tInfl(:,ic)*2/tpanlenInfl(ic), stdbw, nxc, at(iA)*2/tpanlenInfl(ic));
        anx = baryeval(s_aux.t((ic-1)*p+(1:p))*2/(s_aux.thi(ic)-s_aux.tlo(ic)), stdbw, nxc, at(iA)*2/(s_aux.thi(ic)-s_aux.tlo(ic)));
        sA = struct('x',ax,'ws',aws,'w',aw(iA),'sp',asp,'nx',anx);  % Alpert sourse;
%         matA = SLPmatrix(tA,sA);
        matA = SphereQuadKtp(sA,tA);
        potAx(:,(c-1)*p+1:c*p) = matA(:,1:end/2)*L; 
...
i = s.np; % last panel
t2 = struct('x',s.x(end-p+1:end),'nx',s.nx(end-p+1:end)); s_aux2 = struct('x',s_aux.x(end-(n_split+1)*p+1:end),'ws',s_aux.ws(end-(n_split+1)*p+1:end),'nx',s_aux.nx(end-(n_split+1)*p+1:end)); % Alpert source;
% A_aux2 = SLPmatrix(t2,s_aux2);  % A_aux2_0 = A_aux2; 
A_aux2 = SphereQuadKtp(s_aux2,t2); A_aux2_0 = A_aux2; 
for j=1:p
    indj = (i-1)*p+j; % global index of the j-th local node
    % figure out which panel it is located in s_aux
    i_aux = 1; while s.t(indj) > s_aux.thi(i_aux), i_aux=i_aux+1; end
...
alpDLP = SphereQuadKtp(s,s);
alpDLP([1:p,numel(s.x)+(1:p)],[1:2*p,numel(s.x)+(1:2*p)]) = A_aux1;
alpDLP([(s.np-1)*p+(1:p),numel(s.x)+(s.np-1)*p+(1:p)],[(s.np-2)*p+(1:2*p),numel(s.x)+(s.np-2)*p+(1:2*p)]) = A_aux2;

for i = 2:s.np-1
%     keyboard
    alpDLP([(i-1)*p+(1:p),numel(s.x)+(i-1)*p+(1:p)],[(i-2)*p+(1:3*p),numel(s.x)+(i-2)*p+(1:3*p)])=A.s{i};   
end
