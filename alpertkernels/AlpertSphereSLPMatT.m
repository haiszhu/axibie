function alpSLP = AlpertSphereSLPMatT(s)
% traction self eval
% purely numerical
%
% Hai 04/23/20
...
i = 1; % 1st panel
t1 = struct('x',s.x(1:p),'nx',s.nx(1:p)); s_aux1 = struct('x',s_aux.x(1:(n_split+1)*p),'ws',s_aux.ws(1:(n_split+1)*p)); % Alpert source;
% A_aux1 = SLPmatrix(t1,s_aux1);  A_aux1_0 = A_aux1;  
A_aux1 = SphereQuadtpT(s_aux1,t1); A_aux1_0 = A_aux1; 
for j=1:p
    indj = (i-1)*p+j; % global index of the j-th local node
...
i = s.np; % last panel
t2 = struct('x',s.x(end-p+1:end),'nx',s.nx(end-p+1:end)); s_aux2 = struct('x',s_aux.x(end-(n_split+1)*p+1:end),'ws',s_aux.ws(end-(n_split+1)*p+1:end)); % Alpert source;
% A_aux2 = SLPmatrix(t2,s_aux2);  % A_aux2_0 = A_aux2; 
A_aux2 = SphereQuadtpT(s_aux2,t2); A_aux2_0 = A_aux2; 
for j=1:p
    indj = (i-1)*p+j; % global index of the j-th local node
    % figure out which panel it is located in s_aux
    i_aux = 1; while s.t(indj) > s_aux.thi(i_aux), i_aux=i_aux+1; end
